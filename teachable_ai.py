"""Teachable-Machine Bilderkennung für kath.fund.

Primärmodell: ``keras_model.h5`` aus https://github.com/kumma-git/TestKI4
(224x224 RGB, Normalisierung x/127.5 - 1, 11 Klassen aus ``labels.txt``).
...
``None`` zurück und die App nutzt ONNX-/Heuristik-Fallback.
"""
...
LABELS_FILE = Path("teachable_labels.txt")

IMAGE_SIZE = 224

# --------------------------------------------------------------------------
# Mapping: 11 Teachable-Klassen -> 8 Fundbüro-Kategorien (Katharineum)
# --------------------------------------------------------------------------
TEACHABLE_TO_CATEGORY = {
    "Kleidungsstücke": "Kleidung & Textilien",
    "Schulsachen": "Schulmaterial & Bücher",
    "Trinkflasche": "Trinkflaschen & Brotdosen",
    "Brotdose": "Trinkflaschen & Brotdosen",
    "Regenschirm": "Sonstiges",
    "Schlüssel": "Schlüssel & Wertsachen",
    "Kopfhörer": "Elektronik & Kabel",
    "Powerbank/Ladekabel": "Elektronik & Kabel",
    "Brille": "Schlüssel & Wertsachen",
    "Geldtasche": "Schlüssel & Wertsachen",
    "Taschenrechner": "Schulmaterial & Bücher",
}
...
def load_teachable_labels() -> list[str]:
    path = ensure_file(LABELS_FILE, TESTKI4_LABELS_URL)
    if path is not None:
        try:
...
def imagenet_labels() -> list[str]:
    """Optionale ImageNet-Labels für den ONNX-Fallback (app-seitig)."""
    p = Path("imagenet_labels.json")
    if p.exists():
        try:
